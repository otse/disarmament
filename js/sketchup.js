...
(function (sketchup) {
    ...
    async function bakeMaterialFromTuple(name, tuple) {
        ...
        if (tuple[8] && true) {
            const texture = await createTextureFromImage(`${tuple[1]}_displacement.png${salt}`, 8);
            texture.wrapS = texture.wrapT = THREE.RepeatWrapping;
    ...
    async function objectTakesMat(object, index) {
        ...
        if (!mat)
            await bakeMaterialFromTuple(current.name, [current.color, '', 1]);
        mat = mats[current.name];
        if (index == -1)
            object.material = mat;
        else
    ...
    async function objectsTakeMats(group) {
    ...
    sketchup.loadLevel = loadLevel;
    function rotateGunForGripping(group) {
        group.rotation.x = -Math.PI / 2;
        group.rotation.y = Math.PI / 2;
        group.updateMatrix();
    }
    sketchup.rotateGunForGripping = rotateGunForGripping;
    async function loadGun(name) {
        const group = new THREE.Group();
        const loadingManager = new THREE.LoadingManager(function () {
        });
        const colladaLoader = new ColladaLoader(loadingManager);
